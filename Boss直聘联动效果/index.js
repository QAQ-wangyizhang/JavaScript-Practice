...
var selectObj = {
    li_province: null,
    li_city: null,
    arr: ["北京市", "重庆市", "福建省", "江苏省", "广东省", "辽宁省", "内蒙古", "山西省", "青海省", "四川省", "贵州省", "云南省", "陕西省", "西藏", "宁夏", "新疆", "广西", "海南省", "湖南省", "湖北省", "河南省", "山东省", "江西省", "安徽省", "浙江省", "上海市", "黑龙江省", "吉林省", "甘肃省", "天津市", "河北省"],
    ...
    dragA: document.getElementsByClassName('dragArea')[0],
    p_node: document.getElementsByClassName('selectName')[0],
    ...
    showCity: function () {
        ...
        for (var j = 0; j < this.li_province.length; j++) {
            this.li_province.index = j;
            this.li_province[j].addEventListener('moseenter', this.overFn.bind(this))
        }

    },
    overFn: function (e) {
        for (var j = 0; j < this.li_province.length; j++) {
            this.li_province[j].style.backgroundColor = 'fff';
            e.target.backgroundColor = 'f1f3f6';
        }

        var cityStr = '';
        var cityArr = this.city[e.target.index];
        for (var i = 0; i < cityArr.length; i++) {
            cityStr += '<li>' + cityArr[i][1] + ' </li> ';
        }
        this.cityNode.innerHTML = cityStr;
        this.li_city = this.cityNode.getElementsByTagName('li');
        for (var k = 0; k < this.li_city.length; k++) {
            // this.li_city.index = k;
            this.li_city[k].addEventListener('click', this.cityClick.bind(this))
        }
    },
    cityClick: function () {
        var str = e.target.innerHTML;
        this.p_node.innerHTML = str;
        this.dragA.style.display = 'none';
        this.icon.style.backgroundPosition = '0' + '-2067';
        this.bool = true;
    },
    showProvince: function () {
        var self = this;
        this.p_node.onclick = function () {
            if (self.bool) {
                self.dragA.style.display = 'none'
                this.icon.style.backgroundPosition = '0' + '-2044';
                self.bool = true;
            } else {
                self.dragA.style.display = 'block'
                this.icon.style.backgroundPosition = '0' + '-2067';
                self.bool = false;
            }
